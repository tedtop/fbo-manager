...
import { AlertTriangle, ArrowUpDown, Droplets, FileText, Fuel, Plus, Trash2 } from 'lucide-react'
import { Badge } from '@/components/ui/badge'
import { Button } from '@/components/ui/button'
import { Card } from '@/components/ui/card'
import { cn } from '@/lib/utils'
import { useSession } from '@/hooks/use-session'
import { useDeleteTruckSheet, useFuelTrucks, useTruckSheets } from '@/hooks/use-truck-sheets'
...
function sheetStats(sheet: TruckSheetWithReadings) {
  ...
  const dispensed = fuelings.reduce((sum, r) => sum + (r.gallons_pumped ?? 0), 0)
  const tankFills = sheet.readings.filter((r) => r.reading_type === 'tank_fill').length
  ...
  return { dispensed, fuelings: fuelings.length, tankFills, transfers, warnings }
}
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      {trucks.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {trucks.map((truck) => {
            const isJetA = /jet/i.test(truck.equipment_name) || /jet/i.test(truck.notes)
            return (
              <Card key={truck.id} className="p-3">
                <div className="flex items-center gap-2">
                  <Droplets
                    className={cn('w-4 h-4', isJetA ? 'text-sky-400' : 'text-emerald-400')}
                  />
                  <span className="font-mono font-semibold">{truck.equipment_id}</span>
                </div>
                <div className="text-xs text-muted-foreground mt-1 truncate" title={truck.equipment_name}>
                  {truck.equipment_name.replace(/^Fuel Truck \S+\s*/, '') || truck.equipment_name}
                </div>
      ...
      {isLoading ? (
        <div className="text-muted-foreground text-sm py-12 text-center">Loading truck sheets…</div>
      ) : sheets.length === 0 ? (
        <Card className="p-12 text-center space-y-3">
          <FileText className="w-10 h-10 mx-auto text-muted-foreground" />
          <div className="font-semibold">No truck sheets yet</div>
          <p className="text-sm text-muted-foreground">
            Photograph tonight&apos;s truck sheets and import them — the meter readings are extracted
            automatically.
          </p>
        ...
        byDate.map(([date, dateSheets]) => {
          const dayTotal = dateSheets.reduce((sum, s) => sum + sheetStats(s).dispensed, 0)
          return (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation()
                            if (confirm(`Delete truck ${sheet.truck_number} sheet for ${date}?`)) {
                      ...
                      <div className="mt-3 grid grid-cols-3 gap-2 text-center">
                        <div>
                          <div className="text-xl font-semibold">{stats.dispensed.toFixed(0)}</div>
                          <div className="text-[10px] text-muted-foreground uppercase">
                            gal dispensed
                          </div>
                        </div>
                        <div>
                          <div className="text-xl font-semibold">{stats.fuelings}</div>
                          <div className="text-[10px] text-muted-foreground uppercase">fuelings</div>
                        </div>
                        <div>
                          <div className="text-xl font-semibold">{stats.tankFills}</div>
                          <div className="text-[10px] text-muted-foreground uppercase">tank fills</div>
                        </div>
                      </div>
                      <div className="mt-3 flex items-center gap-3 text-xs text-muted-foreground">
                        {sheet.fueler_initials && <span>Init: {sheet.fueler_initials}</span>}
                        {stats.transfers > 0 && (
                          <span className="flex items-center gap-1">
                            <ArrowUpDown className="w-3 h-3" /> {stats.transfers} transfer
                            {stats.transfers === 1 ? '' : 's'}
                          </span>
                        )}
                        {stats.warnings > 0 && (
                          <span className="flex items-center gap-1 text-amber-400">
                            <AlertTriangle className="w-3 h-3" /> {stats.warnings} check
                            {stats.warnings === 1 ? '' : 's'}
                          </span>
                        )}
                      </div>
