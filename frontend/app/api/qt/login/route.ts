...
export async function POST(request: NextRequest) {
  try {
    ...
    const loginPageResponse = await fetch(
      'https://go.qttechnologies.com/Portal/Account/Login',
      {
        method: 'GET',
        headers: {
          Accept:
    ...
    let cookieString = ''
    if (initialCookies) {
      cookieString = Array.isArray(initialCookies)
        ? initialCookies.join('; ')
        : initialCookies
    }
    ...
    if (loginResponse.status === 302 || loginResponse.status === 200) {
      ...
      if (loginCookies) {
        ...
        const existingParts = cookieString
          ? cookieString.split(';').map((c) => c.trim())
          : []
    ...
    return NextResponse.json(
      {
        success: false,
        error: `Login proxy error: ${error instanceof Error ? error.message : 'Unknown error'}`
      },
      { status: 500 }
    )
  }
