...
import { createTankReading, findReadingsByTankId, findReadingsByTankIds } from '@/repositories/tank-readings.repo'
...
describe('tank-readings.repo', () => {
  it('findReadingsByTankId excludes readings older than the window (default 7 days)', async () => {
    ...
    await createTankReading(db, { tank_id: tank.tank_id, level: 1, recorded_at: daysAgo(10) })
    await createTankReading(db, { tank_id: tank.tank_id, level: 2, recorded_at: daysAgo(1) })
  ...
  it('findReadingsByTankId respects a custom day window', async () => {
    const tank = await makeTank(db)
    await createTankReading(db, { tank_id: tank.tank_id, level: 1, recorded_at: daysAgo(20) })
  ...
  it('orders readings by recorded_at descending', async () => {
    const tank = await makeTank(db)
    await createTankReading(db, { tank_id: tank.tank_id, level: 1, recorded_at: daysAgo(2) })
    await createTankReading(db, { tank_id: tank.tank_id, level: 2, recorded_at: daysAgo(1) })
  ...
  it('findReadingsByTankIds spans multiple tanks and orders ascending by recorded_at', async () => {
    ...
    await createTankReading(db, { tank_id: tankA.tank_id, level: 10, recorded_at: daysAgo(2) })
    await createTankReading(db, { tank_id: tankB.tank_id, level: 20, recorded_at: daysAgo(1) })

    const readings = await findReadingsByTankIds(db, [tankA.tank_id, tankB.tank_id], 7)
  ...
  it('findReadingsByTankIds with days=0 returns full history, ignoring the window', async () => {
    const tank = await makeTank(db)
    await createTankReading(db, { tank_id: tank.tank_id, level: 99, recorded_at: daysAgo(400) })
