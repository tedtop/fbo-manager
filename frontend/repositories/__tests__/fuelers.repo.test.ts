...
describe('fuelers.repo', () => {
  ...
  it('joins the owning user record on findFuelerById', async () => {
    const user = await makeUser(db, { username: 'fueler-user', first_name: 'Line' })
    const fueler = await createFueler(db, { user_id: user.id, fueler_name: 'Line Fueler' })
  ...
  it('fails to create a fueler pointing at a nonexistent user (FK guard)', async () => {
    await expect(createFueler(db, { user_id: 999_999, fueler_name: 'Ghost' })).rejects.toThrow()
  })
  ...
  it('findActiveFuelers only returns status=active fuelers', async () => {
    ...
    await createFueler(db, { user_id: activeUser.id, fueler_name: 'Active One', status: 'active' })
    await createFueler(db, { user_id: inactiveUser.id, fueler_name: 'Inactive One', status: 'inactive' })
  ...
  it('lists all fuelers ordered by fueler_name regardless of status', async () => {
    ...
    await createFueler(db, { user_id: u1.id, fueler_name: 'Zed', status: 'inactive' })
    await createFueler(db, { user_id: u2.id, fueler_name: 'Amy', status: 'active' })
  ...
  it('updates a fueler', async () => {
    ...
    const fueler = await createFueler(db, { user_id: user.id, fueler_name: 'Original' })
    const updated = await updateFueler(db, fueler.id, { fueler_name: 'Renamed' })
