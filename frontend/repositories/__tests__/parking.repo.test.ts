...
describe('parking.repo', () => {
  ...
  it('activeOnly (default) excludes locations with display_order <= 0', async () => {
    await createParkingLocation(db, { description: 'Active spot', display_order: 5 })
    await createParkingLocation(db, { description: 'Zeroed spot', display_order: 0 })
  ...
  it('activeOnly=false includes zeroed-out locations', async () => {
    ...
    expect(all.map((l) => l.description).sort()).toEqual(['Active spot', 'Zeroed spot'])
  })

  it('softDeleteParkingLocation zeroes display_order instead of removing the row', async () => {
    const loc = await createParkingLocation(db, { description: 'To retire', display_order: 3 })
  ...
  it('updates a parking location', async () => {
    ...
    const updated = await updateParkingLocation(db, loc.id, { description: 'Gate A2' })
