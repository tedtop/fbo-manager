...
const MINUTEMAN_RAMP_CENTER: [number, number] = [
  -114.08587471583614, 46.92183963008256
]
...
// Stored either as a raw ring of coordinates, or as a GeoJSON Polygon geometry
type ParkingPolygon = number[][] | { coordinates: number[][][] }
...
// Normalize a parking location's polygon to a single ring of [lng, lat] pairs,
// whether it was stored as a raw ring or as a GeoJSON Polygon geometry.
function extractPolygonRing(
  polygon: ParkingPolygon | null | undefined
): number[][] | null {
  if (!polygon) return null
  if (Array.isArray(polygon)) return polygon
  return polygon.coordinates[0] ?? null
}

// Helper to calculate feet per pixel at current latitude
function getFeetPerPixel(lat: number, zoomLevel: number): number {
...
export function ParkingMapEnhanced({ configMode, isAdmin }: ParkingMapProps) {
  ...
  const [previewOverrides, setPreviewOverrides] = useState<
    Record<
      number,
      Partial<GenericAircraftMetadata> & { lat?: number; lng?: number }
    >
  >({})
  const [draggingId, setDraggingId] = useState<number | null>(null)
  const draggingIdRef = useRef(draggingId) // Ref for performance optimizations
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      ...
      const shouldPrevent =
        isMutating > 0 || savePendingRef.current.size > 0 || isSavingRef.current
  ...
  const { data: parkingLocationsData, isLoading: isParkingLoading } = useQuery({
    ...
    queryFn: () => findAllParkingLocations(supabase, false),
  ...
  const { data: flightsData } = useQuery({
    ...
    queryFn: () => findAllFlights(supabase)
  })
  ...
  const createLocationMutation = useMutation({
    mutationFn: async (data: Partial<ParkingLocation>) => {
      return await createParkingLocation(supabase, data)
    },
    onMutate: async (newLocation) => {
      await queryClient.cancelQueries({ queryKey: ['parking-locations'] })
      const previousLocations = queryClient.getQueryData<ParkingLocationRow[]>([
        'parking-locations'
      ])

      queryClient.setQueryData<ParkingLocationRow[]>(
        ['parking-locations'],
        (old) => {
  ...
  const updateLocationMutation = useMutation({
    mutationFn: async ({
      ...
      return await updateParkingLocation(supabase, id, data)
    },
  ...
  useEffect(() => {
    ...
    const initialCenter: [number, number] =
      savedLat && savedLng
        ? [Number.parseFloat(savedLng), Number.parseFloat(savedLat)]
        : MINUTEMAN_RAMP_CENTER

    const initialZoomVal = savedZoom
      ? Number.parseFloat(savedZoom)
      : INITIAL_ZOOM
    const initialBearingVal = savedBearing
      ? Number.parseFloat(savedBearing)
      : 40
    ...
    map.current.addSource('parking-locations', {
      ...
      data: {
        ...
        features: parkingLocations
          .filter((loc) => Array.isArray(loc.polygon) && loc.polygon.length > 0)
          .map((loc) => ({
            type: 'Feature' as const,
            geometry: {
              type: 'Polygon' as const,
              coordinates: [loc.polygon as number[][]]
            },
            properties: {
              id: loc.id,
  ...
  useEffect(() => {
    ...
    const updateMarkers = () => {
      ...
      const flightMarkers = document.querySelectorAll<HTMLImageElement>(
        '.aircraft-marker img'
      )
      for (const img of flightMarkers) {
  ...
  useEffect(() => {
    ...
    const flightsByLocation = new Map<number, FlightWithRelations[]>()
    ...
    for (const flight of flights) {
      if (flight.location?.latitude && flight.location.longitude) {
        const aircraftType =
          flight.aircraft?.aircraft_type_icao || DEFAULT_AIRCRAFT_TYPE

        // Calculate offset for stacking aircraft in same location
        const aircraftsAtLocation = flight.location_id
          ? flightsByLocation.get(flight.location_id) || []
          : []
        const indexAtLocation = aircraftsAtLocation.findIndex(
          (f) => f.id === flight.id
        )
        const offsetAngle =
          (indexAtLocation / aircraftsAtLocation.length) * 2 * Math.PI
        const stackRadius =
          aircraftsAtLocation.length > 1 ? 0.00003 * indexAtLocation : 0
        ...
        const baseLng = Number.parseFloat(flight.location.longitude)
        const baseLat = Number.parseFloat(flight.location.latitude)
          ...
          marker.on('dragend', async () => {
            ...
            const locationsWithPolygons = parkingLocations
              ...
              .map((loc) => ({
                ...
                polygon: extractPolygonRing(
                  loc.polygon as ParkingPolygon | null | undefined
                )
              }))
  ...
  useEffect(() => {
    ...
    for (const loc of genericAircraftLocations) {
      activeIds.add(loc.id)

      // Skip update if this aircraft is currently being dragged
      if (draggingIdRef.current === loc.id) continue
      ...
      if (previewOverridesRef.current[loc.id]) {
        metadata = { ...metadata, ...previewOverridesRef.current[loc.id] }
      }
      ...
      if (marker) {
        ...
        const savedPosition = JSON.parse(
          localStorage.getItem(`aircraft_${loc.id}_position`) || 'null'
        )
        const displayLng =
          savedPosition?.lng ??
          previewOverridesRef.current[loc.id]?.lng ??
          Number.parseFloat(loc.longitude || '0')
        const displayLat =
          savedPosition?.lat ??
          previewOverridesRef.current[loc.id]?.lat ??
          Number.parseFloat(loc.latitude || '0')
        ...
        if (!configMode) {
          newMarker.on('dragstart', () => {
            ...
            const dragStartPosition = {
              ...
              lat: pos.lat,
              lng: pos.lng,
              rotation: newMarker.getRotation()
            }

            // Save to undo stack
            const undoStack = JSON.parse(
              localStorage.getItem(`aircraft_${loc.id}_undo`) || '[]'
            )
  ...
  }, [mapLoaded, genericAircraftLocations, selectedAircraftId, configMode])
  ...
  useEffect(() => {
    const handleBeforeUnload = async (e: BeforeUnloadEvent) => {
      ...
      markersRef.current.forEach((marker, id) => {
        ...
        const dbLat = Number.parseFloat(aircraft.latitude || '0')
        const dbLng = Number.parseFloat(aircraft.longitude || '0')

        const threshold = 0.000001
        if (
          Math.abs(markerPos.lat - dbLat) > threshold ||
          ...
          handleUpdateAircraft(id, {
            ...
            console.error(
              'Failed to save aircraft position before unload:',
              err
            )
          })
        }
  ...
  const handleSelectLocation = (location: ParkingLocation) => {
    ...
    if (location.polygon && map.current) {
      ...
      const coordinates = extractPolygonRing(location.polygon)
  ...
  const scheduleSave = (id: number, marker: mapboxgl.Marker) => {
    ...
    const existingTimeout = savePendingRef.current.get(id)
    if (existingTimeout) {
    ...
    const timeout = setTimeout(async () => {
      ...
      let metadata: Partial<GenericAircraftMetadata>
      try {
  ...
  // Keep the latest scheduleSave available to effects that intentionally
  // don't re-attach their listeners on every render (e.g. the keyboard
  // handler below, which only re-attaches when selectedAircraftId changes).
  ...
  const handleUpdateAircraft = async (
    ...
    const updatePayload: Partial<ParkingLocation> = {
  ...
  const positionBeforeMoveRef = useRef<{
    id: number
    lat: number
    lng: number
    rotation: number
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 'z' && selectedAircraftId) {
        ...
        const undoStack = JSON.parse(
          localStorage.getItem(`aircraft_${selectedAircraftId}_undo`) || '[]'
        )
        ...
        if (undoPosition && undoPosition.id === selectedAircraftId) {
          ...
          localStorage.setItem(
            `aircraft_${selectedAircraftId}_undo`,
            JSON.stringify(undoStack)
          )
          ...
          if (marker) {
            ...
            // Schedule database save after 1.5s delay
            scheduleSaveRef.current(selectedAircraftId, marker)
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
        ...
        if (
          ...
          if (
            positionBeforeMoveRef.current &&
            positionBeforeMoveRef.current.id === id
          ) {
            const undoStack = JSON.parse(
              localStorage.getItem(`aircraft_${id}_undo`) || '[]'
            )
          ...
          if (marker) {
            ...
            scheduleSaveRef.current(id, marker)
          }
        }
      }
    }
  ...
  const handleVerifyData = () => {
    ...
    if (dbCount === markerCount) {
      alert(
        `Integrity Check Passed: ${dbCount} aircraft in DB, ${markerCount} on map.`
      )
    } else {
      alert(
        `Integrity Check FAILED: ${dbCount} aircraft in DB, but ${markerCount} on map!`
      )
      console.error(
        'Missing Aircraft:',
        parkingLocations.filter((l) => !markersRef.current.has(l.id))
      )
    }
  }

  // Derive selected aircraft with preview overrides for real-time updates
  const selectedAircraftRaw = parkingLocations.find(
    (l) => l.id === selectedAircraftId
  )
  ...
  if (selectedAircraftRaw) {
    let metadata: Partial<GenericAircraftMetadata>
    try {
      metadata = selectedAircraftRaw.description
        ? JSON.parse(selectedAircraftRaw.description)
        : {}
    } catch (e) {
      metadata = {}
    }
    ...
    selectedAircraftProps = {
      ...
      aircraftType:
        preview?.aircraftType ?? metadata.aircraftType ?? DEFAULT_AIRCRAFT_TYPE,
      tailNumber:
        preview?.tailNumber ??
        metadata.tailNumber ??
        selectedAircraftRaw.location_code ??
        undefined,
  ...
  // Normalize DB rows (loose/nullable jsonb + text fields) into the strict
  // shape ParkingLocationPanel expects
  const panelLocations = parkingLocations.map((loc) => ({
    id: loc.id,
    location_code: loc.location_code ?? '',
    description: loc.description ?? '',
      ...
      <div className="relative w-full h-full flex flex-col">
        ...
        {!configMode && (
          <div className="absolute top-4 left-4 z-10 flex flex-col gap-2">
            <Button
              onClick={handleAddGenericAircraft}
              className="bg-blue-600 hover:bg-blue-700 text-white shadow-md select-none"
            >
          ...
          <div className="absolute top-4 left-1/2 transform -translate-x-1/2 bg-background/95 backdrop-blur px-4 py-2 rounded-lg shadow-lg border space-y-2 z-50">
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              <input
                ...
                className="w-32"
                onChange={(e) => {
                  const bearing = Number.parseInt(e.target.value)
              ...
              />
              <span className="text-muted-foreground w-16 font-mono font-bold">
                {currentBearing}°
              </span>
              <Button
                variant="outline"
