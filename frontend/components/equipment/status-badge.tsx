...
const STATUS_CONFIG: Record<EquipmentStatus, { label: string; dot: string; badge: string }> = {
  available:      { label: 'Available',      dot: 'bg-green-500',  badge: 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-300' },
  in_use:         { label: 'In Use',         dot: 'bg-yellow-500', badge: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300' },
  maintenance:    { label: 'Maintenance',    dot: 'bg-yellow-500', badge: 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/40 dark:text-yellow-300' },
  out_of_service: { label: 'Out of Service', dot: 'bg-red-500',    badge: 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-300' },
}

const ALL_STATUSES: EquipmentStatus[] = ['available', 'in_use', 'maintenance', 'out_of_service']
      ...
      {open && onStatusChange && (
        <div className="absolute left-0 top-full z-20 mt-1 min-w-[150px] rounded-md border border-border bg-popover shadow-md">
          {ALL_STATUSES.map(s => (
            ...
            >
              <span className={cn('h-2 w-2 rounded-full flex-shrink-0', STATUS_CONFIG[s].dot)} />
