...
export const E2E_SUPABASE_URL =
  process.env.E2E_SUPABASE_URL ?? LOCAL_SUPABASE_URL
...
export function assertSafeE2ETarget(url: string): void {
  ...
  if (process.env.E2E_ALLOW_REMOTE !== 'yes-i-am-sure') {
    throw new Error(
      `Refusing to run E2E setup against "${url}" — this seeds real auth users and drives the real app against it. Only safe against the local Supabase stack (127.0.0.1). Set E2E_SUPABASE_URL and E2E_ALLOW_REMOTE=yes-i-am-sure to override deliberately.`
    )
  }
}
...
export const DEV_USERS = {
  admin: {
    email: 'admin@fbo.local',
    password: 'devadmin',
    role: 'Administrator' as const
  },
  user: {
    email: 'user@fbo.local',
    password: 'devuser',
    role: 'Line Technician' as const
  }
}
